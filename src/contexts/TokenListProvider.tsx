...
const TokenListContext = createContext<TokenListContextValue | undefined>(undefined);
...
export function TokenListProvider({ children }: { children: React.ReactNode }) {
      ...
      (async () => {
        ...
        while (attempt < 5 && !success) {
          try {
            ...
            console.warn(`[TokenList] ⚠️ Fetch attempt ${attempt} failed, retrying...`);
        ...
        if (!success) {
          ...
          setTokens(sorted);
          setError(true);
          console.error('[TokenList] ❌ All fetch attempts failed, using fallback');
        }
        setLoading(false);
      })();
    }
  ...
  const value: TokenListContextValue = {
